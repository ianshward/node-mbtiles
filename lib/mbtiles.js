...
MBTiles.prototype.migrate = function(dryrun, callback) {
    if (!this.open) return;

    // Pre-spec-metadata - assume 1.1
    if (!this._info.spec && this._info.formatter) {
        if (this._info.formatter.indexOf('function(o,d) {') == -1 ||
            this._info.formatter.indexOf('}}[o.format](d); }') == -1) {
            return callback(new Error('Existing formatter not underscore-generated'));
        }
        try {
            var args = {}, a, mbtiles_formatter,
                r = /,obj\.(\w+),/g,
                formats = [
                    {format: 'teaser'},
                    {format: 'full'},
                    {format: 'location'}],
                template_string = '';
            while ((a = r.exec(this._info.formatter)) !== null) {
                args[a[1]] = ('{{' + a[1] + '}}');
            }
            eval('mbtiles_formatter =' + this._info.formatter);
            formats.map(function(f) {
                template_string += '{{#' + f.format + '}}';
                template_string += mbtiles_formatter(f, args);
                template_string += '{{/' + f.format + '}}';
            });
            if (!dryrun) {
                this.startWriting(function(err) {
                    if (err) callback(err);
                    this.putInfo({
                        ...
                        if (err) {
                            ...
                            callback(null, { message: 'file migrated.' });
                        }
                    });
                }.bind(this));
            } else {
                callback(null, { message: 'file migrated. (dryrun)' });
            }
        } catch(e) {
            callback(e);
        }
        ...
        callback(null, {
            message: 'file did not require migration'
        });
    }
};
...
MBTiles.prototype.putInfo = function(data, callback) {
    ...
    // Valid keys.
    var keys = ['name', 'type', 'description', 'version',
        'formatter', 'template',
        'bounds', 'center', 'minzoom', 'maxzoom', 'legend',
        'attribution', 'spec'];
